...
**向后兼容**：如果 `animationIntents` 为空数组（旧格式 script.md），回退到旧模式——自由编排 step 动画。
...
| 有 animationIntents 却自由编排 step | **强制按 animationIntents 指令执行**，step N 对应 animationIntents[N]，不得自由发挥 |
| 把多个 animationIntent 合并为一个 step | 每条 animationIntent 对应一个独立的 step，不得合并 |
| 自行添加 animationIntent 中没有的 step | step 数必须等于 animationIntents 数，不得多加 |
