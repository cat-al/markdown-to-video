...
1. **逐段合成** — 连续 `>` 行合并为一段（空行分隔段落），每段独立生成一个音频文件，确保段级时长精确
...
> 第一段字幕文案，对应第一个 step 的完整口播。
> 段内可以多行 > 书写，连续 > 行自动合并为一段。

> 第二段字幕文案，空行分隔代表新的一段。

> 第三段字幕文案，每段生成一个 TTS 音频。
```
...
读取 Markdown 文案，提取所有场景及其字幕段落：

- 场景标题匹配：`/^## 场景(\d+)[：:]\s*(.+)$/`（兼容中英文冒号）
- 字幕段落：连续 `>` 行（中间无空行）合并为一段，空行分隔段落
- 同一个 `## 场景N` 下的所有 `>` 段都属于该场景
- 非 `## 场景N` 标题下的 `>` 行忽略
...
| 一次性把整个场景文本送 TTS | 逐段调用（每段 = 连续 `>` 行合并），确保段级时长精确 |
