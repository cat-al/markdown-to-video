...
| 色彩 | 语义 | 参考色值 |
|------|------|----------|
| 陶土棕 | 正常 / 起点 / 主要元素 | `var(--color-accent)` `#7A4F2A` |
| 暖金棕 | 强调 / 过渡 / 需要注意 | `var(--color-accent-warm)` `#B8802E` |
| 赤陶红 | 错误 / 终点 / 失控 | `var(--color-danger)` `#8B2515` |
| 橄榄绿 | 辅助 / 聚焦 / 次要元素 | `var(--color-focus)` `#4A6741` |
...
- 紧贴图形节点，间距 8-16px
- 字号小于主视觉元素，但**最小 28px**（手机可读性要求），颜色用 `--color-text-muted`
- 可用细线（`stroke-width: 2`）从标注连到节点

### 数字锚点的美学

大字号数字是视觉重心之一：
- `font-size: 96px` 起步，重要数字可到 `160px+`
- 用 `font-family: var(--font-display)` 让数字有辨识度
- 搭配小字说明（`font-size: 28-36px`），形成大小对比
- 数字滚动动画用 CSS `counter` 或简单 JS，不要过度复杂

### 手机可读性约束

视频最终在手机上以竖屏播放 16:9 横屏内容，画面被 ~5 倍缩放。创作时必须遵守最小字号基线（见 `SKILL.md` "手机可读性"段落），核心原则：
- **所有可见文字 ≥ 28px**（标注、说明等），标题 ≥ 64px
- **连线 stroke-width ≥ 3px**，描边 ≥ 2px
- 字号不够大时拆页，不缩小字号
- 色彩前景与 `#FAF3E9` 背景的明度差 ≥ 45%
